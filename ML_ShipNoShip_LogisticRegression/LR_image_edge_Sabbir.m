...
data_total = feature_vector_edge(p,:); %change here to use feature_vector_edge / feature_vector_gray / feature_vector_rgb
...
N_test = (N_total/K);
rho = 0.01; %learning rate, rho = 1 gives best result for grayscale;rho=0.01 for edge
lambda = 0.1; %regulariztion parameter lambda =0.1 default fro grayscale/edge
iter = 2000; % for gradient_descent; can be more or less depends on convergence 2000 for edge; 1000 for gray
...
for q = 1:K
        ...
        %for RBF
        s = 0.1; % for gray use s=1 and for edge use s=0.1
        Phi = zeros(m,m-1);
        count = 0;
        timeStart4(q)=tic;
        for i = 1:m
...
%	    Phi_test = [ones(mt,1) X_test]; %without basis
