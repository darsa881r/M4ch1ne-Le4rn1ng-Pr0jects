...
N_test = (N_total/K);
rho = 1; %learning rate, rho = 1 gives best result for grayscale;rho=0.01 for edge
lambda = 0.1; %regulariztion parameter lambda =0.1 default for grayscale/edge
...
%        disp(['Cost mimization time for K=',num2str(q),' : ',num2str(timeElapsed5(q))])
        ...
        N_mis_train(q) = 2240-sum(double(k(:,q) == y));
...
%% Prediction
        ...
        N_mis_test(q) =  560-sum(double(k_test(:,q) == y_true));
